...
async function callClaudeAPI(prompt) {
    ...
    if (!ANTHROPIC_API_KEY) {
        console.warn('⚠️ ANTHROPIC_API_KEY not found - skipping Claude validation');
        return null;
    }
...
async function validateAssignmentWithClaude(assignment, existingDatabase) {
    const prompt = `You are helping validate YouTube lecture assignments for a college course management system.

ASSIGNMENT TO REVIEW:
- Video Title: "${assignment.title}"
- Assigned to: ${assignment.classKey.toUpperCase()} Week ${assignment.week}, Lecture ${assignment.lecture}
- Stream Date: ${assignment.streamDate}
- Video URL: ${assignment.videoUrl}

VALIDATION CHECKLIST:
1. Cross-class validation: Does the video title match the assigned class?
   - AVC 185 videos should only go to avc185
   - AVC 200 videos should only go to avc200  
   - AVC 240 videos should only go to avc240
   - AVC 285 videos should only go to avc285

2. Week validation: Does Week ${assignment.week} seem reasonable for stream date ${assignment.streamDate}?

3. URL validation: Is this a valid YouTube URL?

4. Logic check: Does this assignment make sense overall?

Respond with EXACTLY one of:
- "APPROVED: Assignment looks correct"
- "REJECTED: [specific reason why it's wrong]"

Be concise but specific about any issues found.`;

    console.log(`🤖 Requesting Claude validation for: ${assignment.title}`);
    
    const response = await callClaudeAPI(prompt);
    
    if (!response) {
        // Claude API unavailable - fall back to rule-based validation
        console.log('🔄 Falling back to rule-based validation');
        return { approved: true, reason: 'Claude unavailable, using failsafe validation' };
    }
    
    const approved = response.includes('APPROVED');
    const reason = approved ? response : response.replace('REJECTED:', '').trim();
...
async function sendNotificationEmail(assignments) {
    ...
    const subject = `✅ YouTube Auto-Assignment Success - ${new Date().toLocaleDateString()}`;
    const body = `
🎯 Successfully Assigned ${assignments.length} Lectures:

${assignments.map(a => `✅ "${a.title}"
   → ${a.classKey.toUpperCase()} Week ${a.week}, Lecture ${a.lecture}
   → Validated by Claude AI ✓`).join('\n\n')}

All assignments were validated and committed successfully.
View updated lecture library: https://caseyfarina.github.io/lecture-redirects/admin.html

🤖 Automatically generated by YouTube Stream Monitor with Claude validation
`;

    console.log('📧 Success notification would be sent:', { subject, body });
    // Email implementation would go here using nodemailer or similar
}

async function sendBlockedAssignmentsNotification(blockedAssignments) {
    if (!blockedAssignments || !blockedAssignments.length) return;
    
    const subject = `🚨 Lecture Assignments BLOCKED by Claude AI - ${new Date().toLocaleDateString()}`;
    const body = `
⚠️  Claude AI blocked ${blockedAssignments.length} potentially incorrect assignment(s):

${blockedAssignments.map(blocked => `
🚫 "${blocked.assignment.title}"
   Was going to: ${blocked.assignment.classKey.toUpperCase()} Week ${blocked.assignment.week}, Lecture ${blocked.assignment.lecture}
   Claude's reason: ${blocked.reason}
   
   Please review manually: Is this assignment correct?
`).join('\n')}

🔧 ACTION REQUIRED:
1. Review the blocked assignments above
2. If they're correct, use the admin panel to assign them manually
3. If Claude was right, no action needed - bad assignments were prevented!

Admin Panel: https://caseyfarina.github.io/lecture-redirects/admin.html
GitHub Actions Log: https://github.com/caseyfarina/lecture-redirects/actions

🤖 Your AI quality control system is working to prevent assignment errors!
`;

    console.log('🚨 Blocked assignments notification would be sent:', { subject, body });
    // Email implementation would go here using nodemailer or similar
}
...
async function main() {
    ...
    try {
        ...
        for (const stream of streams) {
            ...
            // Prepare assignment for validation
            const assignment = {
                title,
                classKey,
                week,
            ...
            // 🤖 CLAUDE VALIDATION - Per-assignment granularity
            console.log(`🔍 Validating assignment: ${title} → ${classKey.toUpperCase()} Week ${week}-${lecture}`);
            const claudeValidation = await validateAssignmentWithClaude(assignment, indexContent);
            
            if (!claudeValidation.approved) {
                console.error(`🚫 Claude REJECTED assignment: ${claudeValidation.reason}`);
                console.error(`   Video: "${title}"`);
                console.error(`   Intended for: ${classKey.toUpperCase()} Week ${week}, Lecture ${lecture}`);
                
                // Add to blocked assignments for reporting
                if (!assignments.blocked) assignments.blocked = [];
                assignments.blocked.push({
                    assignment,
                    reason: claudeValidation.reason
                });
                
                continue; // Skip this assignment but continue processing others
            }
            
            console.log(`✅ Claude APPROVED assignment: ${claudeValidation.reason}`);
            
            // Update content (only for approved assignments)
        ...
        const blockedCount = assignments.blocked ? assignments.blocked.length : 0;
        
        console.log(`\n📊 ASSIGNMENT SUMMARY:`);
        console.log(`✅ Approved: ${approvedCount} assignments`);
        console.log(`🚫 Blocked: ${blockedCount} assignments`);
        
        // Save changes if any assignments were approved
        if (approvedCount > 0) {
            writeIndexContent(indexContent);
            console.log(`✅ Committed ${approvedCount} approved assignments to database`);
            
            // Send success notification email (Monday/Wednesday/Friday)
        ...
        // Send blocked assignments notification if any
        if (blockedCount > 0) {
            await sendBlockedAssignmentsNotification(assignments.blocked);
        }
        
        if (approvedCount === 0 && blockedCount === 0) {
            console.log('No new assignments processed - action completed successfully');
        }
